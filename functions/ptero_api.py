...
def con_create_allocation(apikey, nodeid, ip, port):
    url = 'https://games.plox.host/api/application/nodes/{0}/allocations'.format(nodeid)
    headers = {
...
def con_user_create(apikey, email, first_name, last_name):
    url = 'https://games.plox.host/api/application/users'
    headers = {
        "Authorization": "Bearer {0}".format(apikey),
        "Accept": "application/json",
        "Content-Type": "application/json",
...
def con_create_server(apikey, name, user, nestid, egg, memory, swap, disk, io, cpu, databases, backups, allocations, allocation_id):
    ...
    url = 'https://games.plox.host/api/application/nests/{0}/eggs/{1}?include=variables'.format(nestid, egg)
    headers = {
        "Authorization": "Bearer {0}".format(apikey),
        "Accept": "application/json",
        "Content-Type": "application/json",
   ...
   # exit(data['attributes'])
    ...
    for i in data['attributes']['relationships']['variables']['data']:
        ...
        envdata.append(default)
# TODO: Match the values of env data & default value

    url = 'https://games.plox.host/api/application/servers'
    headers = {
        "Authorization": "Bearer {0}".format(apikey),
        "Accept": "application/json",
        "Content-Type": "application/json",
    ...
    payload = {'name': '{0}'.format(name), 'user': '{0}'.format(user), 'egg': '{0}'.format(egg), 'docker_image': '{0}'.format(docker_image), 'startup': '{0}'.format(startup_cmd), 'limits': {'memory': '{0}'.format(memory), 'swap': '{0}'.format(swap), 'disk': '{0}'.format(disk), 'io': '{0}'.format(io), 'cpu': '{0}'.format(cpu)}, 'feature_limits': {'databases': '{0}'.format(databases), 'backups': '{0}'.format(backups), 'allocations': '{0}'.format(allocations)}, 'allocations': {'default': '{0}'.format(allocation_id)} }
    response = requests.request('POST', url, json=payload, headers=headers)
